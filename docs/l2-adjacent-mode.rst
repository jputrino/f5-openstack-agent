...
.. index::
   single: openstack-bigip-ctlr; L2-adjacent mode

L2-adjacent mode
================

L2-adjacent mode (:code:`f5_global_routed_mode = False`) is the |agent-long|'s default mode.
The |agent-long| **does not automatically detect any network or BIG-IP configurations**.
You must provide the appropriate L2/L3 network settings for your BIG-IP device(s) in the :ref:`L2 segmentation mode <l2-segmentation-settings>` and :ref:`L3 segmentation mode <l3-segmentation-settings>` sections of the |agent| :ref:`configuration file <agent-config-file>`.
...
The |agent-long| can also manage BIG-IP Virtual Edition (VE) instances deployed 'over the cloud' (or :term:`overcloud`) using L2-adjacent mode.
...
.. important::
   ...
   The |agent-long| L2/L3 segmentation mode settings must match the configurations of your existing external network and BIG-IP device(s).

Prerequisites
-------------

- VLANs and VxLAN or GRE tunnels configured as as appropriate for your environment.
  If you're using GRE or VxLAN tunnels, you must have a `Better or Best license`_ that supports SDN.

.. warning::

   Many L3 segmentation mode parameters depend on other configuration parameters.
   Read the text in the :ref:`agent configuration file` carefully before changing these settings to ensure they don't conflict.
...
Tells the |agent-long| what type of tunnel(s) connect the BIG-IP device(s) to controller/compute node(s) in OpenStack (GRE or VxLAN).
...
.. important::
   ...
   If you're managing :term:`overcloud` BIG-IP VE instances, uncomment this line in the agent configuration file.
...
- If this is your initial launch, :ref:`start the F5 OpenStack BIG-IP Controller <topic-start-the-agent>`.
- If you have updated the configurations for a running |agent| instance, restart the service:

  - :command:`systemctl systemctl start f5-openstack-agent`   \\ CentOS
  - :command:`service f5-oslbaasv2-agent start`               \\ Ubuntu

See the `F5 OpenStack LBaaSv2 user documentation`_ for more information about the F5 OpenStack LBaaS Integration.
