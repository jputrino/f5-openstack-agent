...
The |agent-long| (``openstack-bigip-ctlr``) is an OpenStack `Neutron plugin agent <https://docs.openstack.org/security-guide/networking/architecture.html>`_.
It works in conjunction with the `F5 OpenStack LBaaSv2 Driver </products/openstack/lbaasv2-driver/latest/index.html>`_ to manage F5 BIG-IP `Local Traffic Manager <https://f5.com/products/big-ip/local-traffic-manager-ltm>`_ (LTM) services via the OpenStack Neutron API.

.. seealso::

   For more information about how the |agent-long| interacts with the Neutron API and BIG-IP devices, see :ref:`Architecture`.


.. index::
   triple: openstack-bigip-ctlr; downloads; debian
   triple: openstack-bigip-ctlr; downloads; rpm
...
.. index::
   single: openstack-bigip-ctlr; install
...
.. tip::
   ...
   You can use the `f5-openstack-ansible`_ project to deploy the |agent-long|, |driver-long|, and all project dependencies.
   See `Deploy OpenStack Agent and Driver with Ansible`_ for more information.

.. index::
   triple: openstack-bigip-ctlr; install; debian
...
.. index::
   triple: openstack-bigip-ctlr; install; pip
...
.. tip::
   ...
   .. parsed-literal::
      ...
      $ sudo pip install |f5_agent_pip_url_branch|


.. index::
   triple: openstack-bigip-ctlr; install; rpm
...
#. Install all three (3) packages.
   ...
   .. parsed-literal::
      ...
      $ sudo rpm -ivh |f5_icontrol_rpm_package| |f5_sdk_rpm_package| |f5_agent_rpm_package|

Next Steps
``````````

- `Install the F5 OpenStack LBaaSv2 Driver </products/cloud/f5-openstack-lbaasv2-driver/latest/index.html#installation>`_.
- :ref:`Configure the OpenStack BIG-IP Controller <configure-the-f5-openstack-agent>`.

.. index::
   single: openstack-bigip-ctlr; architecture
...
The |agent-long| translates the Neutron LBaaS API calls to iControl REST API calls and `configures the requested objects`_ on the BIG-IP device(s) identified in the :ref:`agent configuration file <agent-config-file>`.

When the |agent-long| and |driver-long| run on your OpenStack Neutron Controller, you can use the standard ``neutron lbaas`` commands to manage BIG-IP LTM objects. [#neutroncli]_
...
.. index::
   single: openstack-bigip-ctlr; supported features
...
.. index::
   single: openstack-bigip-ctlr; configure
...
#. Start the |agent-long|.
   ...
   Once you have configured the |agent-long|, you can use the appropriate command(s) for your OS to start or stop the agent service.
...
The :ref:`agent configuration file <agent-config-file-example>` (:file:`/etc/neutron/services/f5/f5-openstack-agent.ini`) tells the |agent-long| about the network architecture and how/where the BIG-IP device(s) fit in.
...
The latter impacts how the |agent-long| configures BIG-IP objects in response to Neutron API calls.
...
Each section below corresponds to a section of the :ref:`agent configuration file`.

.. index::
   single: openstack-bigip-ctlr; default settings
...
.. index::
   single: openstack-bigip-ctlr; environment settings
...
.. index::
   single: openstack-bigip-ctlr; static agent configuration settings
...
.. index::
   single: openstack-bigip-ctlr; device settings
...
.. index::
   single: openstack-bigip-ctlr; L2 segmentation mode settings
...
.. index::
   single: openstack-bigip-ctlr; L3 segmentation mode settings
...
.. index::
   single: openstack-bigip-ctlr; device driver/iControl driver settings
...
.. index::
   single: openstack-bigip-ctlr; certificate manager settings
...
.. index::
   single: openstack-bigip-ctlr; configuration file examples
...
.. index::
   single: openstack-bigip-ctlr; unsupported features
...
.. index::
   single: openstack-bigip-ctlr; upgrade
...
Perform the steps below on every server running |agent|.
...
#. Copy the |agent| configuration file to a different directory (for example, :file:`~/f5-upgrade-temp`).
...
#. Move or rename the |agent| log file.

   Your new |agent-long| will not start if it finds an existing agent log file.
...
#. Stop and remove the current version of |agent|.
...
#. Follow the :ref:`installation <agent-installation>` instructions to install a different version of |agent|.
...
.. [#agent] Similar to BIG-IP :term:`high availability`, but applies to the |agent-long| processes.
