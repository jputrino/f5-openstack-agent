...
The F5 OpenStack BIG-IP Controller is an OpenStack `Neutron plugin agent <https://docs.openstack.org/admin-guide/networking-arch.html#overview>`_. It works in conjunction with the `F5 OpenStack LBaaSv2 Driver <http://clouddocs.f5.com/products/openstack/lbaasv2-driver/latest/index.html>`_ to manage F5 BIG-IP `Local Traffic Manager <https://f5.com/products/big-ip/local-traffic-manager-ltm>`_ (LTM) services via the OpenStack Neutron API.
...
The F5 OpenStack BIG-IP Controller is compatible with OpenStack releases from Liberty forward.
...
Installing openstack-bigip-ctlr
-------------------------------

Please see `installing the F5 BIG-IP Controller </docs/README.rst#installation>`_.
...
When the you install ``openstack-bigip-ctlr``, the ``debug_bundler.py`` script installs itself in ``/usr/bin/f5/``.
...
Please file an `issue <https://github.com/F5Networks/openstack-bigip-ctlr/issues>`_ in this repository.
